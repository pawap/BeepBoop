...
public class LevelTest {
	...
	public void isPositionFree_blockedByNonwalkableTile_returnFalse() {
		...
		level.getLandscape().place(nonwalkablePosition.x, nonwalkablePosition.y, TileFactory.ROCK_0);
		// act
		boolean result = level.isPositionFree(nonwalkablePosition.x, nonwalkablePosition.y);
		// assert
		assertFalse(result);
	}
	...
	public void isPositionFree_blockedByThing_returnFalse() {
		...
		// act
		boolean result = level.isPositionFree(positionOfThing.x, positionOfThing.y);
		// assert
		assertFalse(result);
	}
	...
	public void isPositionFree_blockedByPlayer_returnFalse() {
		...
		// act
		boolean result = level.isPositionFree(origin.x, origin.y);
		// assert
		assertFalse(result);
	}
	...
	public void addThing_noOtherThingAtSamePosition_newThingAdded() {
		...
		level.addThing(someThing);
		// assert
		assertTrue(level.getThing(positionOfThing.x, positionOfThing.y).equals(someThing));
	}
	...
	public void moveThing_movePossible_thingIsAtNewPosition() {
		...
		// assert
		assertTrue(someThing.equals(level.getThing(newPosition.x, newPosition.y)));
	}
	...
	public void moveThing_movePossible_thingIsNotAtOldPosition() {
		...
		level.moveThing(oldPosition, newPosition);
		// assert
		assertFalse(someThing.equals(level.getThing(oldPosition.x, oldPosition.y)));
	}
	...
	private Landscape createTenByTenGrassLandscape() {
		...
		someLandscape.placeRect(0, 0, landscapeSize.width - 1, landscapeSize.height - 1, TileFactory.GRASS_0);
	...
	private Thing createThingAt(Point positionOfThing) {
		int someAmount = 42;
		Thing thing = new Resource(someAmount, TileFactory.NULL_TILE, "someType");
